...
Библиотека состоит из двух вещей:
...
колонок — это около 440 байт, модель поверх него — около 690. Файл в
...
|                                | 500 000 строк, 41 МБ |
|--------------------------------|----------------------|
| `cursor()` по всей таблице     | 787 мс, 0 МБ         |
| `count()`                      | 64 мс, 0 МБ          |
| `orderByDesc('id')->limit(10)` | 1039 мс, 0 МБ        |
| `paginate(10)`                 | 66 мс, 0 МБ          |
