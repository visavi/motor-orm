...
Библиотека состоит из трёх слоёв, из которых вы пишете против двух:

|             | что это                                   | что на ней держится                                        |
|-------------|-------------------------------------------|------------------------------------------------------------|
| **Модель**  | `class Article extends Model`, `$article` | таблица, касты, scope, связи — и значения одной строки      |
| **Запрос**  | `Article::query()`                        | условия, сортировка, пагинация, запись                     |
| **Таблица** | `Table`, `CsvFile`, `RowMapper`, …        | байты, колонки, касты, отбор и сортировка строк            |

Модель объявляет таблицу, а прочитанная — она же и строка этой таблицы: всё, что
строка умеет отвечать, живёт методом модели. Чего она не несёт — так это запроса,
который её нашёл: условия остаются в `Query`, поэтому строка не читает таблицу
сама по себе.

Под ними чтение разделено дальше, по классу на задачу: `Table` и `CsvFile` —
файл, `RowMapper` — строки и значения, `Conditions`, `Sorter`, `KeySearch`,
`TableWriter` и `RelationLoader` — остальное. Их не называют по имени: из них
состоит `Query`.
...
|--------------------------------|-------------------|-------------------|-----------|
| найти запись по ключу          | 67.7 мс, 0.6 МБ   | 0.7 мс, 0.6 МБ    | **×0.01** |
| посчитать строки по условию    | 66.1 мс, 0.6 МБ   | 71.4 мс, 0.6 МБ   | ×1.08     |
| выбрать строки по условию      | 67.3 мс, 0.6 МБ   | 71.9 мс, 0.7 МБ   | ×1.07     |
| страница из десяти строк       | 0.1 мс, 0.6 МБ    | 0.1 мс, 0.6 МБ    | ×1.32     |
| десять последних с сортировкой | 132.5 мс, 33.1 МБ | 97.3 мс, 0.6 МБ   | **×0.73** |
| обойти всю таблицу             | 66.4 мс, 0.6 МБ   | 81.4 мс, 0.6 МБ   | ×1.23     |
| прочитать таблицу целиком      | 69.1 мс, 29.6 МБ  | 93.7 мс, 35.4 МБ  | ×1.36     |

Цена ORM на сканировании — меньше десятой доли времени, и она уходит на то, ради
чего её берут: разбор условий, приведение типов, объекты вместо массивов.

Поиск по ключу выпадает из этого ряда: `find()` не читает таблицу вовсе, а
[делит файл пополам](#поиск-по-ключу), и проход по всем строкам проигрывает ему
на два порядка.

Строка с сортировкой выбивается: ORM быстрее сырого кода, а памяти тратит
0.6 МБ против 33. Он собирает таблицу в память и сортирует её целиком, а запрос
с `limit` несёт только те строки, которые попадут в ответ.
...
колонок — это около 440 байт, модель поверх него — около 790. Файл в
41 МБ, прочитанный целиком, займёт 377 МБ.

Поэтому упирается не размер таблицы, а размер результата:

|                                | 500 000 строк, 41 МБ |
|--------------------------------|----------------------|
| `cursor()` по всей таблице     | 797 мс, 0 МБ         |
| `count()`                      | 62 мс, 0 МБ          |
| `orderByDesc('id')->limit(10)` | 987 мс, 0 МБ         |
| `paginate(10)`                 | 64 мс, 0 МБ          |
| `get()` всей таблицы           | 377 МБ               |
...
Второй потолок — время: проход стоит около 1.1 мкс на строку, и `where` без
попадания в начало файла на 500 000 строк займёт около полусекунды. От этого спасают
...
[коллекцию](#коллекции) моделей, пустую если ничего не нашлось.

Прочитанная модель держит значения одной строки и умеет записать себя обратно,
но не несёт условий и сама запросов не выполняет:
...
$new = new Article();               // строка, которой ещё нет в файле
$new->title = 'Новый заголовок';
$new->save();                       // вставлена, ключ выдала таблица
```
...
Всё, что строка таблицы умеет отвечать о себе, — метод модели, рядом с
колонками, которые она читает:
...
Связь читается в свою модель так же, поэтому `$article->user` отвечает всё, что
умеет `User`.

### Поиск по ключу

`find()` таблицу не читает. Ключи выдаются по возрастанию, а перезапись
сохраняет порядок строк, поэтому файл обычно лежит отсортированным по первой
колонке — и до нужной строки можно дойти делением файла пополам. Два десятка
чтений вместо всех строк: на таблице в 50 000 строк запись из конца находится
за 2.1 мс вместо 73.

Ничего не принимается на веру. Найденная строка должна нести именно запрошенный
ключ и начинаться там, где начинается запись. Второе проверяется точно: значение
может держать перенос строки внутри кавычек, и тогда следующая строка файла
выглядит записью, ею не будучи. Кавычка открывает значение, следующая закрывает,
кавычка внутри значения удваивается — значит перед началом настоящей записи их
чётное число. Байты при этом только считаются, а не разбираются, и весь файл до
найденной строки обходится примерно в полсотни раз дешевле, чем читается.

Не сошлось — `find()` молча читает таблицу построчно, как раньше. Поэтому
отсортированности файла никто не требует: таблица с ключами вразнобой,
нечисловой ключ, битая строка, сломанные кавычки — всё это даёт прежний полный
проход и прежний ответ, просто без ускорения. Ошибиться строкой поиск не может:
любое сомнение стоит скорости, а не правильности.

Деление работает только на голом поиске по ключу. Условия, сортировка или
смещение, выставленные до `find()`, означают, что таблицу всё равно надо
прочитать:

```php
Article::query()->find(1);                          // делением пополам
Article::query()->where('name', 'Миша')->find(1);   // полным проходом
```

Отсутствующий ключ тоже стоит полного прохода: пропуск и файл, отсортированный
иначе, снаружи выглядят одинаково, а разойтись в ответе они не должны.
...
Оттуда, откуда скажет приложение, и больше ниоткуда: библиотека, читающая csv,
про запросы ничего не знает и глобалок сама не трогает. Пока ей не сказали,
имеется в виду первая страница.

`page()` задаёт страницу явно:

```php
Article::query()->page(3)->paginate(10);
```

`resolvePageUsing()` говорит, откуда берёт страницу запрос, которому её не
назвали, — один раз, в бутстрапе приложения. Замыкание получает имя параметра,
а значение, которое числом не является, считается первой страницей:
...
/* самый простой вариант, для приложения, живущего на $_GET */
Pagination::resolvePageUsing(static fn (string $name) => $_GET[$name] ?? 1);
```

Имя параметра — одно на приложение: его получает резолвер, с ним же строятся
ссылки.
...
объекту эти настройки не могут. `resolvePageUsing(null)` убирает источник.

### Пагинация без подсчёта

Общее число строк — это то, что покупает нумерованные ссылки, и платят за него
проходом по всей таблице. На таблице в 50 000 строк в этом и состоит вся
стоимость страницы: выбрать десять строк первой страницы стоит 0.05 ms,
пересчитать остальные — 6.

Подсчёт без условий строки не разбирает, а только считает, поэтому платить
приходится куда меньше, чем раньше. Но проход остаётся проходом, и на поздних
страницах, где к нему добавляется чтение до нужного смещения, разница между
двумя пагинациями почти пропадает.
